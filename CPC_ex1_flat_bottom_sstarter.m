...
for ii = 1:mmax
    ...
    Aj = wmode_zs*WAPEsolveSSP_PML_SelfStarter(PEpars);
...
TLc_selfst = interp2(Xx,Yy,TLxy,xc,yc);

figure; 
hold all; 
plot(180*th_c/pi,TLc,'linewidth',1.5); 
plot(180*th_c/pi,TLc_selfst,'linewidth',1.5,'linestyle','--');
xlim([-90 90]);
ylim([-80 -50]);
legend('analytical solution','SSP + Self starter');

dlmwrite('selfst_analyt.txt',[180*th_c.'/pi TLc.' TLc_selfst.'],'delimiter','\t','precision',6);
